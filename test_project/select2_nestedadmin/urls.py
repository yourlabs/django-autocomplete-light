...
class LinkedDataView(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        # We don't really care about filtering in this test app, but only that the values are forwarded properly.
        level_one = self.forwarded.get('level_one', None)
        level_two = self.forwarded.get('level_two0', None)

        if not level_one or not level_two:
            raise NestedAdminError('Linked fields are not forwarded properly for nested admin')
