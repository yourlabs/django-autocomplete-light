...
class AdminForeignKeyTestCase(Select2Story, case.AdminMixin, case.OptionMixin,
    ...
    def test_twidget_is_not_initialized(self):
        ...
        # First check that the setup script ran
        # in order to make sure that we don't have a false positive on the next assertion.
        setup_succeeded = self.browser.evaluate_script('window.__dal__tSelect2Setup')
        self.assertTrue(setup_succeeded)

        initialize_called = self.browser.evaluate_script('window.__dal__tSelect2Initialized')
