...
var forwardHandlerRegistry = forwardHandlerRegistry || {};

function registerForwardHandler(name, handler) {
    forwardHandlerRegistry[name] = handler;
}

function getForwardHandler(name) {
 ...
 * field was found
 */
function getFieldRelativeTo(element, name) {
...
function getValueFromField(field) {
    ...
    if (strategy === 'multiple') {
        return serializedField.map(function (item) {
...
function getForwards(element) {
    ...
    $.each(forwardList, function(ix, field) {
        ...
        if (field.type === 'const') {
            ...
            forwardedData[dstName] = getValueFromField(element);
            ...
            var forwardedField = getFieldRelativeTo(element, srcName);

            if (!forwardedField.length) {
                return;
            }

            forwardedData[dstName] = getValueFromField(forwardedField);
        } else if (field.type === 'javascript') {
            var handler = getForwardHandler(field.handler);
